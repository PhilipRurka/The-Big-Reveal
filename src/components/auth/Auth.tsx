...
const Auth = forwardRef<Refs, AuthProps>(
  (
    ...
    { emailRef, passwordRef, usernameRef }: any
  ) => {
    return (
      <AuthStyled>
        <AuthTitle id={AuthTransitionIdsEnum.TITLE}>{title}</AuthTitle>
        <Form>
        ...
        {toAuthLinks && (
          <ToAuthLinkStyled id={AuthTransitionIdsEnum.TO_AUTH_LINKS}>
            {toAuthLinks.map(({ href: toLinkHref, title: toLinkTitle }) => (
              <ToAuthLinkItem key={toLinkTitle} href={toLinkHref}>
                {toLinkTitle}
              </ToAuthLinkItem>
            ))}
          </ToAuthLinkStyled>
